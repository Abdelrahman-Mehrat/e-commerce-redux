...
export const shoppingCartSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromCart: (state,action) => {
        ...
        state.shoppingTotalPrice -= (action.payload.price * action.payload.quantity)
      state.shoppingQuantity = state.shoppingProducts.length
    },
    removeAllCart:(state)=>{
      state.shoppingProducts = []
      state.shoppingTotalPrice = 0
      state.shoppingQuantity = 0
    },
    ...
    changeProductQuantity:(state,action)=>{
      ...
      shopItems.forEach(e => {
        if(e.id == changedProduct.product.id){
...
export const { addToCart,removeFromCart,removeAllCart,changeProductQuantity} = shoppingCartSlice.actions;
